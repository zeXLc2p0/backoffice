...
The scripts in this directory will create linelists of ID3C and REDCap data for submission to the Washington Department of Health.
See the specification for the linelist data format under `./bin/wa-doh-linelists/docs/SFS_DOH_Linelist_Data_Formats.xlsx`.

The `./bin/wa-doh-linelists/generate` script is the top-level script that exports ID3C data, merges it with REDCap report data, and performs transformations all in one.
...
The output linelists are stored in the `backoffice` repo under `~/backoffice/bin/wa-doh-linelists/data/linelist_{DATE}.csv`.
...
We have to point our `generate` command to a database.
...
3. **python 3.6**
   ...
   if you don't have it installed, one way to do it is with [pyenv](https://github.com/pyenv/pyenv).
...
Repeat this process for every SCAN REDCap project you're targeting in the [linelist configuration file](https://github.com/seattleflu/backoffice/blob/master/etc/wa-doh-linelists.yaml) (also at `~/backoffice/etc/wa-doh-linelists.yaml`).
